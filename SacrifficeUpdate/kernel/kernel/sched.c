...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *the_pool, arrays[3]; //hw2
...
static inline void enqueue_task(struct task_struct *p, prio_array_t *array)
{
	...
	if(p->policy == SCHED_POOL){
		...
		printk("enqueue_task print\n");
		printk("nr_running:%d\n", rq->nr_running);
		printk("the_pool num:%d\n", rq->the_pool->nr_active);
		printk("active num:%d\n", rq->active->nr_active);
		printk("expired num:%d\n", rq->expired->nr_active);
	}
	*/
...
static inline void activate_task(task_t *p, runqueue_t *rq)
{
	...
	if (p->policy == SCHED_POOL){ //hw2
		//printk("change array to pool\n");
		enqueue_task(p, rq->the_pool);
		//printk("back from pool enqueue\n");
		p->activation_start = jiffies; 
		rq->nr_running++; 
		return;
	}
...
static inline void deactivate_task(struct task_struct *p, runqueue_t *rq)
{
	...
	p->total_activation_time += (jiffies - p->activation_start);
	p->array = NULL;
}
...
int sys_get_remaining_timeslice(pid_t pid){
	//TODO: PID = 0 
	//printk("here remaining time slice\n");
	task_t* p = find_task_by_pid(pid);
	if(!p)
		return -ESRCH;
	//printk("time_slice 1 %d\n", p->time_slice );
	//printk("policy:%d\n",p->policy);
	...
	//printk("time_slice 2 %d\n", p->time_slice );
	return p->time_slice;
}



int sys_get_total_proccesor_usage(pid_t pid){
	task_t* p = find_task_by_pid(pid);
	if(!p){
		return -ESRCH;
	}
...
int sys_get_total_time_in_runqueue(pid_t pid){
	...
	if (p->state == TASK_RUNNING){
		return jiffies - p->activation_start;
	}
...
repeat_lock_task:
	...
	if (!p->array) {
		 ...
		 */
		if ((rq->curr->policy != SCHED_POOL && p->policy != SCHED_POOL) || (rq->curr->policy == SCHED_POOL && p->policy == SCHED_POOL)){
			if (p->prio < rq->curr->prio)
				resched_task(rq->curr);
		}
		else if (rq->curr->policy == SCHED_POOL && p->policy != SCHED_POOL){
			resched_task(rq->curr);
		}
...
void wake_up_forked_process(task_t * p)
{
	...
	p->cpu = smp_processor_id();
	int parent_pid = p->pid;
	task_t* parent = find_task_by_pid(parent_pid);
	if(parent != NULL && parent->policy == SCHED_POOL){
		p->prio = parent->prio;
	}
	activate_task(p, rq);
...
int sys_sacrifice_timeslice(pid_t pid){
	...
	if(!p)
		return -ESRCH;
	if(p->pid == current->pid || p->policy == SCHED_FIFO || p->state == TASK_ZOMBIE || current->policy == SCHED_POOL)
		return -EINVAL;
...
int sys_search_pool_level(pid_t pid, int level){
	...
	if(!p)
		return -ESRCH;
	runqueue_t *rq = this_rq_lock();
	list_t* head = rq->the_pool->queue + level;

	if(p->array == rq->the_pool){
		//printk("in the pool\n");
	}
	...
	list_for_each(iter, head){
		int found_pid = list_entry(iter, task_t, run_list)->pid;
		if (found_pid == pid){
			rq_unlock(rq);
			return counter;
		}
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (p->array != rq->active && p->policy != SCHED_POOL) {
		set_tsk_need_resched(p);
		return;
	}
	/* hw2 */
	...
	*/
	if (p->policy == SCHED_POOL && time_pool == 0){
		set_tsk_need_resched(p);
	...
	spin_lock(&rq->lock);
	if (p->policy != SCHED_POOL && unlikely(rt_task(p))) {
		/*
	...
	*/
	if (p->policy != SCHED_POOL){
		...
		if (!--time_pool){
			//printk("time pool remaining: %d", time_pool);
			set_tsk_need_resched(p);
			goto out;
		}
		...
		int idx = sched_find_first_bit(rq->the_pool->bitmap);
		if (idx < p->prio){
			set_tsk_need_resched(p);
			goto out;
		}
...
pick_next_task:
#endif
	if (unlikely(!rq->nr_running) || ( (rq->the_pool->nr_active == rq->nr_running)&&(time_pool==0) ) ) { //hw2
#if CONFIG_SMP
		load_balance(rq, 1);
		if (rq->nr_running)
			goto pick_next_task;
	...
	array = rq->active;
	if (unlikely(!array->nr_active) && rq->expired->nr_active) {
		/*
		...
		array = rq->active;
		rq->expired_timestamp = 0;
	} else if (!array->nr_active && !rq->expired->nr_active && time_pool>0){
		//printk("schedule print, scheduling pool process\n");
		array = rq->the_pool;
	}
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	if (policy < 0){
		...
		if (policy != SCHED_FIFO && policy != SCHED_RR &&
				policy != SCHED_OTHER && policy != SCHED_POOL){
			goto out_unlock;
		}
	}
	...
	if (((policy == SCHED_OTHER) != (lp.sched_priority == 0)) && (policy != SCHED_POOL)){
		goto out_unlock;
	}
	if ((policy == SCHED_POOL)){ //hw2
		if((lp.sched_priority<0) || (lp.sched_priority>MAX_PRIO-1)){
			goto out_unlock;
		}
	}
	retval = -EPERM;
	...
	else if (policy != SCHED_OTHER && policy != SCHED_POOL){ //hw2
		p->prio = MAX_USER_RT_PRIO-1 - p->rt_priority;
		/*
		if (old_policy == SCHED_POOL){
			p->array = our_rq->active;
		}*/
			
	}
	else{
		p->prio = p->static_prio; //hw2
		/*
		if (old_policy == SCHED_POOL){
			p->array = our_rq->active;
		}*/
...
/*
	if(p->policy == SCHED_POOL){
		//printk("returning from setscheduler \n");
		set_need_resched();
	}
	*/
	return retval;
...
asmlinkage long sys_sched_yield(void)
{
	...
	if (current->policy == SCHED_POOL || unlikely(rt_task(current))) {
